...
> ℹ️ Palette CMS v1.11.2 以上では、全コンテンツの一括アップロードは「同期」コマンドへ統合されています（[(10) コンテンツの同期](#10-コンテンツの同期) 参照）。
...
「同期」ビューの「同期」ボタンを押すと、接続先のサーバーコンテンツとローカルコンテンツを突き合わせて差分を検出し、操作を選択して一括同期できます。

タブ別に分類:

- **更新対象**: ローカルとサーバーの両方に存在 → デフォルトで「更新」
- **置き換え選択**: ローカルのみに存在 → 「新規作成」または「置き換え」を選択
- **サーバーのみ**: サーバーのみに存在 → 「削除」または「取得」を選択

「実行」ボタンで順序付けて一括実行されます。実行結果は同期ログとしてタブで開かれます。

主に **本番環境の複製として作成した検証・開発環境** で、git で管理しているローカルコンテンツとサーバーを整合させる用途を想定しています。
